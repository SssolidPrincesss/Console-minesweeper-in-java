...
Также он возвращает булевое значение, которое нужно для работы главного цикла  
•	CheckArrConainsDark  
...
•	main  
Главный метод, в котором находится главный цикл (в моем случае do-while, т. к. сначала игрок делает ход, а потом уже происходит проверка на мину и количество свободных ходов)   
В первую очередь здесь создается ссылка на объект MessageHelper, а затем вызыввается его метод, отвечающий за приветствие.  
...
Еще создается целочисленная переменная для подсчитывания колличества ходов игрока.  
...
Данная игра является демо-версией нормального «Сапера» по двум причинам:     
...
В будущем я бы хотел дополнить эту игру графическим интерфейсом в необычном оформлении, возможностью пройти игру на сайте, а также как фишку добавить возможность создавать не только четырехугольные поля ("необычная" форма поля будет даваться игроку на выбор, в отличие от обычного четырехугольного).
